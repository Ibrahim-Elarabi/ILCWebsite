...
namespace ILCWebsite.Areas.Admin.Controllers
{
    ...
    public class ServiceHomeController : Controller
    {
        ...
        {
            var ServiceHome = _unitOfWork._serviceHomeRepo.FindOne(d=>d.Id == id && d.IsDeleted != true);
            var result = _mapper.Map<ServiceHomeVM>(ServiceHome);
            return View(result);
        }
        ...
        public async Task<JsonResult> Edit(EditServiceHomeVM model)
        {
            try
            {
                ...
                else
                {
                    ...
                    _unitOfWork._serviceHomeRepo.Update(_mapper.Map<ServiceHome>(model));
